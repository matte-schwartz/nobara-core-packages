...
if [[ $MEDIAFIXUP == 1 ]]; then
        if [[ -n $DISPLAY ]] ; then
		if zenity --question --text="Multimedia Codec packages are missing or not installed properly! These packages are needed for video playback (decoding) and encoding. Would you like to fix this?" 2>/dev/null
                then
                        media_fixup
                else
...
snap_install_progress() {
	if [[ -n $DISPLAY ]] ; then
		if zenity --question --text="Snap has been detected! Would like to update all Snaps on your system?" 2>/dev/null
		then
			snap refresh
...
if cat /tmp/dnf.sync.success ; then
    if [[ $DNF_STATE_STAGE == true ]]; then
		...
		if [[ -n $DISPLAY ]] ; then
			if zenity --question --title='Update my system' --text='It is recommended to reboot for changes to apply properly. Reboot now?' 2>/dev/null
			then
                                echo INFO: Rebooting...;
				dnf_sync_log_remove
				log_cleanup
				...
				dnf_sync_log_remove
				log_cleanup
				/etc/nobara/scripts/nobara-updater/end.sh
			fi
		else
			...
			* )
			...
			;;
                ...
                log_cleanup
		if [[ -n $DISPLAY ]] ; then
			if zenity --info --title='Update my system' --text='No updates required, your system is already up to date!' 2>/dev/null
			then
				/etc/nobara/scripts/nobara-updater/end.sh
			fi
		else
